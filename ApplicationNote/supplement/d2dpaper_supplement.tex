...
software package, the CVODES solver \citep{Hindmarsh:2005fb} is used. 
CVODES solves both stiff and non-stiff systems and, for reasons of
efficiency, is implemented in C. The methods used in CVODES 
...
\begin{equation}
	L(\boldsymbol{\theta}) = \sum_{k=1}^m \sum_{j=1}^{d_k}\, 2\log(\sqrt{2 \pi}) + 
2\log(\sigma_k(t_j, \boldsymbol{\theta}))\, + \left(\frac{y_{kj}^\dagger - y_{k}(t_{j}, 
\boldsymbol{\theta})}{\sigma_k(t_j, \boldsymbol{\theta})}\right)^2 \label{llhoodfun2}
\end{equation}
...
data and normally distributed measurement noise is assumed. The index $k$ runs over all 
experimental measurements, the index $j$ over all time points for the measurements. For 
simplicity, the sum over the different data sets is omitted here. 
The variance can be a 
parameterized function $\sigma_k(t_j, \boldsymbol{\theta})^2$, and the corresponding 
...
method applies a second order Taylor expansion $\tilde L$ of the objective function
\begin{equation}
...
\begin{eqnarray}
	L(\boldsymbol{\theta}) & = & const + \sum_{k=1}^m \sum_{j=1}^{d_k}\, 
2\log(\sigma_k(t_j, \boldsymbol{\theta}))\, +  \left(\frac{y_{kj}^\dagger - y_{k}(t_{j}, 
\boldsymbol{\theta})}{\sigma_k(t_j, \boldsymbol{\theta})}\right)^2 \label{llhoodfun3ij} \\
	& = & const + \sum_{q=1}^{s} \tilde r_{q}(\boldsymbol{\theta})^2 + \sum_{q=1}^{s}  
r_{q}(\boldsymbol{\theta})^2 \label{llhoodfun3}
\end{eqnarray}
where the new sum index $q$ runs over all $k$ and $j$ in ($\ref{llhoodfun3ij}$) and $s = 
\sum_{k=1}^m d_k$. The first sum contains the elements 
\begin{equation}
	\tilde r_{q}(\boldsymbol{\theta}) = \sqrt{2\log(\sigma_k(t_j, \boldsymbol{\theta}))}. 
\label{rl1}
\end{equation}	
Since $\log(\sigma_k(t_j, \boldsymbol{\theta}))$ can be negative, in practice 
\begin{equation}
	\tilde r_{q}(\boldsymbol{\theta}) = \sqrt{2\log(\sigma_k(t_j, \boldsymbol{\theta})) + c} 
\label{rl2}
\end{equation}
...
\begin{equation}
	r_{q}(\boldsymbol{\theta}) = \frac{y_{ki}^\dagger - y_{k}(t_j, \boldsymbol{\theta})}
{\sigma_k(t_j, \boldsymbol{\theta})}.  \label{rl3}
\end{equation}
...
\end{equation}
Let ${f}_\sigma$ be the equation for the variance $\sigma_k(t_j, \boldsymbol{\theta})^2$. 
