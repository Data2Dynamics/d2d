% Investigate iteration 1_result_294 = i = 297
...
for i = 297
    ...
    if ~isempty(strfind(results(i).name,'_simu_'))
        ...
        try
            ...
            arSave(sprintf('result_%s_manual',results(i).name(22:end)))
        catch exception
            fprintf('%s\n', exception.message);
            arSave(sprintf('result_%s_manual_failed',results(i).name(22:end)))
        end
