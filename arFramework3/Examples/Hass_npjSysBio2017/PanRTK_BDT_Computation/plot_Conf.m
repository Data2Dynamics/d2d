...
for i=1:length(struct_tmp)   
    ...
    %if(~isfield(bdt_figures.(struct_tmp{i})(1).testing(plot_id),'MPV') || isnan(bdt_figures.(struct_tmp{i})(1).testing(plot_id).MPV(1,1,2)))
        ...
        for k=1:length_conf
            ...
            mean_FDR = nansum(bdt_figures.(struct_tmp{i})(1).testing(plot_id).conf_col_mean(is_negative,k,:),1);
            std_FDR = nansum(bdt_figures.(struct_tmp{i})(1).testing(plot_id).conf_col_std(is_negative,k,:).^2,1);
