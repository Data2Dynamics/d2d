...
meritvals.loglik        = 2*ar.ndata + ar.chi2fit;
meritvals.loglik_all    = 2*(ar.ndata+ar.nconstr) + ar.chi2fit + ar.chi2constr;
meritvals.lsqnonlin     = sum([ar.res,ar.constr]);

meritvals.chi2_all      = ar.chi2fit + ar.chi2constr;
meritvals.chi2_res      = sum(ar.res(ar.res_type==1)); % same as ar.chi2 - ar.chi2prior
meritvals.chi2_err      = ar.chi2err;
meritvals.chi2_err_addc = sum(ar.res(ar.res_type==2)); % sum of error residuals
