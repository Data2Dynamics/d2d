...
elseif(ar.config.optimizer == 19)
    ...
    options.Display = 'none';%'iter-detailed';
    ...
    options = optimoptions(@fmincon,'Algorithm',options.Algorithm,'Display',options.Display,'CheckGradients',options.CheckGradients,'SubproblemAlgorithm',options.SubproblemAlgorithm,...
        'HessianApproximation',options.HessianApproximation,'MaxIterations',options.MaxIterations,'StepTolerance',options.StepTolerance,'SpecifyObjectiveGradient',options.SpecifyObjectiveGradient,...
        'SpecifyConstraintGradient',options.SpecifyConstraintGradient);
    [pFit, ~, exitflag, output, lambda, grad] = ...
        fmincon(@merit_fkt_fmincon_laplace, ar.p(ar.qFit==1),[],[],[],[],lb,ub, ...
        myconfun,options);
    resnorm = merit_fkt(pFit);
    jac = [];
    fit.grad = grad;
...
% fmincon for laplace
function [l, g] = merit_fkt_fmincon_laplace(pTrial)
global ar
arCalcMerit(ar.config.useSensis, pTrial);
arLogFit(ar);
l = sum(ar.res);
if(nargout>1)
    g = sum(ar.sres(:, ar.qFit==1),1);
end
