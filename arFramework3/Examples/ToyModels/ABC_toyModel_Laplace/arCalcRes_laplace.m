...
% evaluated:
if  ar.ndata_res>0 && (ar.config.fiterrors==1 || (ar.config.fiterrors==0 && sum(ar.qFit(ar.qError==1)==1)>0) && ar.config.useFitErrorCorrection  )
...
%             if(ar.model(m).data(d).use_laplace)
%                 [ar.model(m).data(d).res, ar.model(m).data(d).chi2]         = fres_laplace(ar.model(m).data(d).yExp, ar.model(m).data(d).yExpSimu, ystd, fiterrors_correction_factor);
%             else
                [ar.model(m).data(d).res, ar.model(m).data(d).chi2, res_tmp]         = fres(ar.model(m).data(d).yExp, ar.model(m).data(d).yExpSimu, ystd, fiterrors_correction_factor);
%             end
            ...
            if (ar.config.fiterrors == 1) || (ar.config.fiterrors==0 && sum(ar.qFit(ar.qError==1)<2)>0 ) % error residuals are only !=0 if errors are fitted:
                [ar.model(m).data(d).reserr, ar.model(m).data(d).chi2err]   = fres_error(ystd);
...
%                 if(ar.model(m).data(d).use_laplace)
%                     ar.model(m).data(d).sres = fsres_laplace(ar.model(m).data(d).syExpSimu, ar.model(m).data(d).yExp, ystd, fiterrors_correction_factor);
%                 else
                    ar.model(m).data(d).sres = fsres(ar.model(m).data(d).syExpSimu, ar.model(m).data(d).yExp, ystd, res_tmp, fiterrors_correction_factor);
%                 end
                if (ar.config.fiterrors == 1) || (ar.config.fiterrors==0 && sum(ar.qFit(ar.qError==1)<2)>0 )
                    [ar.model(m).data(d).sres, ar.model(m).data(d).sreserr] = fsres_error(ar.model(m).data(d).yExp, ar.model(m).data(d).res, ar.model(m).data(d).sres, ystd, systd);
                end
            else
...
function [res,chi2,res_tmp] = fres(yexp, y, ystd, fiterrors_correction_factor)
    res_tmp = (yexp-y)./ystd * fiterrors_correction_factor;
...
function [reserr,chi2err] = fres_error(ystd)

    reserr = log(ystd);    
    reserr(isnan(ystd)) = 0;
    
    chi2err = sum(reserr,1);  
...
%      }
%  }
function [sres,sreserr] = fsres_error(yexp, res, sres, ystd, systd)
sreserr = NaN(size(sres));
    
for ip=1:size(sres,3)
    sres(:,:,ip) = sres(:,:,ip) - (systd(:,:,ip) .* res ./ ystd);
    sreserr(:,:,ip) = systd(:,:,ip) ./ ystd;
