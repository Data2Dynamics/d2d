        ...
        if(ar.model(jm).qPlotXs(jplot)==1)
            if(ar.config.ploterrors == -1)
                [h, fastPlotTmp] = arRaiseFigure(ar.model(jm).plot(jplot), ...
                    ...
                    figcount, fastPlot, 2);
                ar.model(jm).plot(jplot).fighandel_xCI = h;
            else
                [h, fastPlotTmp] = arRaiseFigure(ar.model(jm).plot(jplot), ...
                    'fighandel_x', ['X: ' ar.model(jm).plot(jplot).name], ...
                    figcount, fastPlot, 2);
                ar.model(jm).plot(jplot).fighandel_x = h;
            end
            ...
            if(saveToFile)
                if(ar.config.ploterrors == -1)
                    ...
                    ar.model(jm).plot(jplot).savePath_FigX = arSaveFigure(h, ...
                        ar.model(jm).plot(jplot).name, '/Figures/X');
                end
            end
