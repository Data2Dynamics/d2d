...
function arSubsetSensi()
    ...
    for jm = 1 : numel( ar.model )
        for jc = 1 : numel( ar.model(jm).condition )
            ...
            ar.model(jm).condition(jc).backwardIndices = zeros( numel( ar.model(jm).condition(jc).pLink ), 1 );
    ...
    for jm = 1 : numel( ar.model )
        for jc = 1 : numel( ar.model(jm).ss_condition )
            ...
            ar.model(jm).ss_condition(jc).backwardIndices = zeros( numel( ar.model(jm).ss_condition(jc).pLink ), 1 );
