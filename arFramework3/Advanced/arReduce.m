...
function arReduce( m )
    ...
    for i = 1 : size(ar.model(m).pools.dependent, 2)
        fprintf( '%d: %s\n', i, sprintf( '%s ', ar.model.x{abs(ar.model(m).pools.dependent(:,i))>0} ) );
    end
    ...
    while( ~isempty( ar.model(m).pools.states ) )
        ...
        ar.model(m).p{end+1}    = ar.model.removedStates.totalVariable;
        ar.model(m).fp{end+1}   = ar.model.removedStates.totalVariable;
        ar.model(m).z{end+1}    = removalStruct.p;
        ar.model(m).fz{end+1}   = removalStruct.fp;
        
        % Substitute state variable in rate equations
        fvtemp = arSubs( sym( ar.model(m).fv ), sym(removalStruct.p), sym(removalStruct.fp) );
        ...
        ar.model.N( removedState, : ) = [];
        ar.model.Cm( removedState, : ) = [];
        ar.model.Cm_par( removedState, : ) = [];

        % Remove the states from the initial conditions
        ar.model.px0( removedState ) = [];
        ar.model.xUnits( removedState, : ) = [];
        ar.model.cLink( removedState ) = [];
        ar.model.x( removedState ) = [];
        ar.model.xNames( removedState ) = [];
        ar.model.qPlotX( removedState ) = [];
        ar.model.qPlotZ( end + 1 ) = 1;        
...
function [removedState, removalStruct] = genReplacementRule( m, selected )
    ...
    for js = 1 : numel( other )
        fp = sprintf( '%s + %g * %s', fp, ar.model(m).pools.others(other(js)), ar.model(m).x{other(js)}  );
        otherStates{end+1} = { ar.model(m).pools.others(other(js)), ar.model(m).x{other(js)} };
    end
