function ppl_calc(m, c, jx, xFit, p, t, doPPL, takeY, dir, stepsize, xstd, ed_steps, pReset, chi2start, backward, fineInt)
    ...
    if(size(ar.model(m).(data_cond)(c).(struct_string),1)>length(ar.model(m).(data_cond)(c).tFine))
        ar.model(m).(data_cond)(c).(struct_string)(end:end-(ar.model(m).(data_cond)(c).(struct_string)-ar.model(m).(data_cond)(c).tFine),:) = [];
    end
