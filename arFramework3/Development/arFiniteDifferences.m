...
%
% Calculate sensitivities for fitting by finite difference and store them
% in the field ar.model(jm).(condition/data)(jc/jd).s(x/u/v/y)ExpSimu.
%
% arFiniteDifferences(dp)    
%   dp:     parameter variation to use      [1e-3]
...
try
    ...
    arPop('silent');
    error( 'An error occurred during the evaluation of arFiniteDifferences:\n%s', c.message );
end
