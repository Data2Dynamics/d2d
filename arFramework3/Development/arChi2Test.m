...
if(~isfield(ar.config,'alpha_level'))
    ar.config.alpha_level = 0.05;
end

if(ar.pval>=ar.config.alpha_level)
    fprintf('pval(dof=%i) = %f, model is compliant with data for %5.2f%% sign. level\n', ...
        ar.ndata - sum(ar.qFit==1), ar.pval, (1-ar.config.alpha_level)*100);
else
    fprintf('pval(dof=%i) = %f, model is NOT compliant with data for %5.2f%% sign. level\n', ...
        ar.ndata - sum(ar.qFit==1), ar.pval, (1-ar.config.alpha_level)*100);
end
