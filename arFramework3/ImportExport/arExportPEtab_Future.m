function arExportPEtab_Future(name, export_SBML)
% arExportPEtab_Future(name, export_SBML)
...
%
% Notes:
%    This version reverts the changes made to the condition table logic in commit 0821098.
%
%    This way, we allow mathematical expressions in the condition table, even if this is not
%    strictly PEtab v1 compliant. However, this allows to provide exported files for all benchmark models.
%    We expect this is more usefull to the comunity than strict compliance to PEtab v1,
%    especially since the upcomming PEtab versions v2 plans to support this feature.
%
% References
...
tmpCell = table2cell(condT);
for i = 1:size(tmpCell,1)
    for j = 1:size(tmpCell,2)
        if ~isempty(tmpCell{i,j})
            tmpCell{i,j} = char(arSym(tmpCell{i,j}));
        end
    end
end
condT = cell2table(tmpCell, 'VariableNames', condT.Properties.VariableNames);
qRemove = arrayfun(@(jp) all(strcmp(condT.modelFP(jp), condT{jp, 3:end})), 1:size(condT, 1));
condT = condT(~qRemove, :);
...
for d = 1:length(ar.model(m).data)
    ...
    obsId = ar.model(m).data(d).y';
    IDs.obs{d} = cell(size(ar.model(m).data(d).y')); % for later use
    obsName = ar.model(m).data(d).y';
